...
bash <(wget -qO- "https://raw.githubusercontent.com/kospiotr/us-playground-scripts/${BRANCH}/task-java-install.sh?nocache") > /var/log/task-java-install.log

echo "Running services - will be startup scripts responsibility"
bash <(wget -qO- "https://raw.githubusercontent.com/kospiotr/us-playground-scripts/${BRANCH}/task-consul-run-instance.sh?nocache") > /var/log/task-consul-run.log
